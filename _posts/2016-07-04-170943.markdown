...
On the other hand, when we consider priors that have tails at least as heavy as exponential distributions, i.e., $$-\log\pi(\theta)\asymp \vert\theta\vert$$, one can show that the resulting posterior mean satisfies the following:
