...
![look at me](/assets/penalty2.pdf)
 ...
 It is clear that the $$L_2$$ penalized estimators (right) is not a sparse estimator, which could result in an exactly zero estimated value, since regardless of $$\bar y$$ the estimator $$\theta_{L_2}$$ is always non-zero.
