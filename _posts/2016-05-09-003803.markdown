...
$$ Y_i = \beta_0 + \beta_1 X_{ 1 i } + \beta_2  X_{ 2 i } +\beta_3 X_{3i} + \epsilon_i$$ , where $$\epsilon_i\overset{i.i.d}\sim N(0,1\sigma^2)$$ for $$ i=1,...,n $$.
...
AIC: $$-2\hat l_M + 2|M|$$
BIC: $$-2\hat l_M + \log n |M|$$, 

where $$\hat l_M$$ is the maximized likelihood of the model $$M$$ and  $$|M|$$ is the number of parameters in the model $$M$$.  So the first term $$\hat l_M$$ indicates the goodness-of-fit of the model to the observed data and the second term penalizes the complexity of the model so that more complex model is less likely to be selected.
