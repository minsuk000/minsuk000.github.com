...
$$
\widehat{\theta} = \underset{\theta}{\operatorname{argmax}} -L(y\mid \theta) + p_\lambda(\theta),
$$   
 where $$p_\lambda(\dot)$$ is the penalty on $$\theta$$ with the tuning parameter $$\lambda$$ that controls the strength of the penalty.
